export default async function FetchOffer(tag, variation = 'main') {
  const aempublishurl = process.env.REACT_APP_AEM_PUBLISH;
  const aemauthorurl = process.env.REACT_APP_AEM_AUTHOR;
  const aemurl = `${process.env.REACT_APP_OFFERTAGQUERY_URL}${tag};variation=${variation}?ts=${Math.random()*1000}`;
